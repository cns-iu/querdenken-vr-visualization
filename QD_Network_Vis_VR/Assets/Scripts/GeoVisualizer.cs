...
public class GeoVisualizer : MonoBehaviour
{
    ...
    void CreateNodes()
    {
        ...
        foreach (Entity e in m_Entities)
        {
            ...
            g.AddComponent<Record>().m_EntityType = e.entityType;
            g.GetComponent<Record>().m_Id = e.name;
            g.GetComponent<Record>().lat = e.lat;
            g.GetComponent<Record>().lon = e.lon;
    ...
    void LayOutNodes()
    {
        ...
        {
            m_Marks[i].transform.position = new Vector3(RescaleCoord(m_Marks[i].GetComponent<Record>().lat), m_DisplayHeight, RescaleCoord(m_Marks[i].GetComponent<Record>().lon) * 1.3f);
        }
